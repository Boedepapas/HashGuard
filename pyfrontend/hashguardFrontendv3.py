...
quarantine_frame = [
    ...
    [sg.Button("Delete Selected", key="-DELETE-", button_color=(BTN_FG, BTN_BG), disabled=True)]
]
...
def request_backend_delete(name):
    def worker(name):
        try:
            ok, err = backend.delete_quarantine(name)
        except Exception as e:
            ok, err = False, str(e)
...
while True:
    ...
    if event == "-DELETE-":
        ...
        if len(selected) != 1:
            ...
            if confirm == "Yes":
                ...
                window["-QUARANTINE-"].update(disabled=True)
    ...
    if event == "-QUARANTINE-":
        sel = values.get("-QUARANTINE-")
        if sel and isinstance(sel, (list, tuple)) and len(sel) == 1:
